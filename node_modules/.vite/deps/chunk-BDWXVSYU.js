...
function useListState(initialValue = []) {
  ...
  const append = (0, import_react19.useCallback)((...items) => setState((current) => [...current, ...items]), []);
  const prepend = (0, import_react19.useCallback)((...items) => setState((current) => [...items, ...current]), []);
  const insert = (0, import_react19.useCallback)(
    (index, ...items) => setState((current) => [...current.slice(0, index), ...items, ...current.slice(index)]),
    []
  );
  const apply = (0, import_react19.useCallback)(
  ...
  const pop = (0, import_react19.useCallback)(
    () => setState((current) => {
  ...
  const shift = (0, import_react19.useCallback)(
    () => setState((current) => {
      const cloned = [...current];
      cloned.shift();
      return cloned;
  ...
  const reorder = (0, import_react19.useCallback)(
  ...
  const setItem = (0, import_react19.useCallback)(
  ...
  const setItemProp = (0, import_react19.useCallback)(
  ...
  const applyWhere = (0, import_react19.useCallback)(
    (condition, fn) => setState(
      (current) => current.map((item, index) => condition(item, index) ? fn(item, index) : item)
    ),
    []
  );
  const filter = (0, import_react19.useCallback)((fn) => {
    setState((current) => current.filter(fn));
  }, []);
  const handlers = (0, import_react19.useMemo)(
    () => ({
      setState,
      append,
      prepend,
...
//# sourceMappingURL=chunk-BDWXVSYU.js.map
